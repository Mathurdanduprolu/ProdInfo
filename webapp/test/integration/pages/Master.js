			...
			onTheMasterPage : {
				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("ProductGuid") === oObject.ProductGuid;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].ProductGuid;
										} else {
											sCurrentId = aItemsNotInTheList[0].ProductGuid;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("EtProductSet", {
											ProductGuid : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("ProductGuid"),
				...
				assertions : {
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all EtProductSet to be able to check for the total amount
